...
/*		StringBuffer url=new StringBuffer();
	...
	public static Parameter toParameter(HttpServletRequest request){
		...
		while(em.hasMoreElements()){
			...
			if(values.length==1){
				...
				if("status".equals(name)){
					status=NumberUtils.toInt(values[0]);
					if(status>-1){
						parameter.add(name,status);
					}
				}else{
					parameter.add(name,values[0]);
				}
			}else if(values.length>1){
				parameter.add(name, Arrays.asList(values));
			}
