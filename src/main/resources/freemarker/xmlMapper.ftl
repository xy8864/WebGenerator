...
<mapper namespace="${domainPackage}.${ClassName}Mapper" >
	...
	<#list fieldList as field>
		...
		<#else><result column="${field.name}" property="${field.name}" jdbcType="${field.jdbcType}" /></#if>
	</#list>
	</resultMap>

	<insert id="create">
		insert into ${tableName}
		(<#list fieldList as field>${field.name}${field_has_next?string(",","")}</#list>) values (<#list fieldList as field>
		${r"#{"+field.name+",jdbcType="+field.jdbcType+"}"} ${field_has_next?string(",","")}
	</#list>)
	</insert>
