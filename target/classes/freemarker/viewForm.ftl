...
<body>
	...
	<form:form id="inputForm" modelAttribute="${className}" action="${r"${ctx}"}/${urlPrefix}/save" method="post" class="form-horizontal">
        ...
        <#if field.modifyPage=='Y'>
        <#if !StringUtil.isBlank(field.dictType)>
        <div class="control-group">
			<label class="control-label">${field.comment}:</label>
			<div class="controls">
				<form:select path="${field.name}">
					<form:options items="${r"${"+"fns:getDictList('${field.dictType}')"+r"}"}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<#else>
        <div class="control-group">
			<label class="control-label">${field.comment}:</label>
			<div class="controls">
				<form:input path="${field.name}" htmlEscape="false" maxlength="${field.length}" class="required"/>
			</div>
		</div>
		</#if>
		</#if>
